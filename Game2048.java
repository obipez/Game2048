...
public class Game2048 extends JPanel {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (!myWin && !myLose) {
					...
					case KeyEvent.VK_LEFT:
						left();
						break;
						// TODO: handle code for right, up, and down moves, go to that code
	...
	//move up
	public void up() {
		//TODO: same logic as right but handle up move, made opposites for right/left & up/downs (may or may not work)
		myTiles = rotate(270);
		left();
		myTiles = rotate(90);
	}
	...
	//move down
	public void down() {
		//TODO: same logic as right but handle down move
		myTiles = rotate(90);
		left();
		myTiles = rotate(270);
	}
	...
	//TODO: function to check if the board is full, use the availableSpace function to implement this one
	private boolean isFull() {
		//use availableSpace() here
		if(availableSpace().size() == 0);
		return true;
	}

	//function to check if a move is possible 
	//description: if there are 2 tiles with same values then you can combine them 
	// 			 which means there are moves left to be done
	boolean canMove() {
		// TODO: handle option to return true here 
		if (!isFull()) {
			return true;
		}
		// went through the whole board so can't move, return false
		for (int x = 0; x < 4; x++) {
			for (int y = 0; y < 4; y++) {
				Tile t = tileAt(x,y);
	...
	private Tile[] getLine(int index) {
		...
		// TODO: handle logic to get the elements in the board into result
	...
	File file = new File("Scores2048.txt");
	String myNowHighScore = "";
	public void readFile(String Scores2048) {
		try {
  ...
  private void drawTile(Graphics g2, Tile tile, int x, int y) {
    ...
    // Display high Score 
    // TODO: handle display of high score
    g.setFont(new Font(FONT_NAME, Font.PLAIN, 18));
    g.setColor(new Color(78, 139, 202));
    g.drawString("Highscore: " + myHighScore, 25, 365);
  
   
	  try {
		BufferedWriter output = new BufferedWriter(new FileWriter(file, true));
		  output.newLine();
		  output.append("" + myScore);
		  output.close();
	} catch (IOException e) {
  ...
  static class Tile {
    ...
    public boolean isEmpty() {
    	...
    	//TODO: handle logic to make sure tile is really empty
 	if(value == 0) {
    		return true;
    	}
		return false;
